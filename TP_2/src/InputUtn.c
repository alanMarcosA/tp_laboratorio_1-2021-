...
int utn_getEmployees(char name[],char lastName[],float* salary,int* sector){
	int retorno;
	retorno=-1;
	if(utn_getText(name, 51, "ingrese el nombre", "ingrese correctamente el nombre", REINTENTOS) == 0){
		if(utn_getText(lastName, 51, "ingrese el apellido", "ingrese correctamente el apellido", REINTENTOS)==0){
			if(utn_getFloat(salary, "ingrese el salario", "ingrese correctamente el salario", 0, 100000000, REINTENTOS)==0){
				if(utn_getInt(sector, "ingrese el sector", "ingrese correcamente el sector", -1, 1000, REINTENTOS)==0){
					retorno=0;
				}
			}
		}
